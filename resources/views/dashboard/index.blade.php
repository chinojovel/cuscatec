                            ...
                            <h4 class="mt-1 mb-0">${{ $sumTotalSalesLastSevenDays->total_sales }}
                                @if ($percentageChange > 0)
                                    <sup class="text-success fw-medium font-size-14">
                                        <i class="mdi mdi-arrow-up"></i> {{ $percentageChange }}%
                                    </sup>
                                @else
                                    <sup class="text-danger fw-medium font-size-14">
                                        <i class="mdi mdi-arrow-down"></i>{{ $percentageChange }}%
                                    </sup>
                                @endif
                            </h4>
                            <div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h4 class="mt-1 mb-0">{{ $countTotalOrdersLastSevenDays->total_orders }}
                                @if ($countPercentageChange > 0)
                                    <sup class="text-success fw-medium font-size-14">
                                        <i class="mdi mdi-arrow-up"></i> {{ $countPercentageChange }}%
                                    </sup>
                                @else
                                    <sup class="text-danger fw-medium font-size-14">
                                        <i class="mdi mdi-arrow-down"></i>{{ $countPercentageChange }}%
                                    </sup>
                                @endif
                            </h4>
                            <div>
            ...
            <div class="card">
                ...
                <div class="card-body pt-1">
                    <div class="table-responsive">
                        <table id="sellStatesTable" class="sell-states table table-centered align-middle table-nowrap mb-0"> 
                            <thead>
                                <tr>
                                    <th>State</th>
                                    <th>Sales Cash</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ( $state['percentage_variation'] > 0)
                                                <sup class="text-success fw-medium font-size-14">
                                                    <i class="bx bx-trending-up text-success"></i> {{ $state['percentage_variation'] ? number_format($state['percentage_variation'], 2) . '%' : 'N/A' }}
                                                </sup>
                                            @else
                                                <sup class="text-danger fw-medium font-size-14">
                                                    <i class="bx bx-trending-down  text-danger"></i>{{ $state['percentage_variation'] ? number_format($state['percentage_variation'], 2) . '%' : 'N/A' }}
                                                </sup>
                                            @endif
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        startDate.setDate(startDate.getDate() - 6); // Hace 6 días (incluye el actual)

        // Crear un array para los últimos 7 días, asegurando que la hora sea 00:00:00
        var daysRange = [];
        for (var i = 0; i < 7; i++) {
            var currentDate = new Date(startDate);
            currentDate.setDate(currentDate.getDate() + i);
            currentDate.setHours(0, 0, 0, 0); // Aseguramos que las horas sean 00:00:00
            daysRange.push(currentDate.toISOString().split('T')[0]); // Formato YYYY-MM-DD
        }

        // Crear un mapa con los datos de ventas, usando la fecha como clave
        var salesDataMap = {};
        totalSalesByDayData.forEach(function(item) {
            salesDataMap[item.day] = item.total_sales;
        });

        // Asegurarse de que todos los días aparezcan, incluso con 0 ventas
        var formattedData = daysRange.map(function(day) {
            return {
                x: day, // Usamos el día como X (fecha)
                y: salesDataMap[day] || 0 // Si no hay ventas para ese día, usamos 0
            };
        });
        ...
        // Crear un mapa con los datos de ventas, usando la fecha como clave
        var salesDataMap = {};
        countTotalSalesByDayData.forEach(function(item) {
            salesDataMap[item.day] = item.total_orders;
        });
